...
class Trade:
    ...
    def check_args_len(self, args: list[int]):
        if not len(args) == len(self.payment):
            raise WrongNumberOfArguments(
                f"Wrong number of arguments: entered {len(args)}, required {len(self.payment)}"
            )
    ...
    def interpret_invalid_payment(
        self, playment_flags: list[bool], args: list[int]
    ) -> list[str]:
        res = []
        for i, (payment_flag, payment_item) in enumerate(
            zip(playment_flags, self.payment)
        ):
            if not payment_flag:
        ...
        match self.returns:
            ...
            case ReturnType.RANDOM_NUMS:
                ...
                for _ in range(self.multiplier):
                    to_ret.extend(
                        list_of_randint_N(
                            random.choices([1, 2, 3, 4, 5], weights=[1, 5, 15, 8, 5])[0]
                        )
                    )
                return to_ret
            case _:
