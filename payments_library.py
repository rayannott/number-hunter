...
class Literal(PaymentItem):
    ...
    def difficulty(self) -> float:
        return (
            1.0 / len(self.args) if (1 not in self.args and 2 not in self.args) else 0.2
        )
...
def get_random_payment():
    if random.random() < 0.7:
        return [
            get_random_payment_item()
            for _ in range(random.choices([1, 2, 3], weights=[3, 2, 1])[0])
        ]
