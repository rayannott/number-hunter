...
HELP_STR = [
    ...
    [
        "save",
        "save current state of the game (this is done automatically on 'exit' and 'quit')",
    ],
    [
        "sell *<trade_ids>",
        "give away the chosen trades and get from 1 to 2 random numbers for each of them",
    ],
    [
        "bargain *<args>",
        f"give away {N_FOR_BARGAIN} unique numbers to get one random trade or {N_FOR_MEGA_BARGAIN} to get to choose one trade from 4 random trades",
    ],
    ...
    def execute_command_menu(self, cmds: list[str]):
        match cmds:
            ...
            case ["new", *args]:
                ...
                if name is not None:
                    self.gi = GameInfo(
                        name + ".pi", init_nums=init_nums, init_trades=init_trades
                    )
                else:
            ...
            case ["load"]:
                if not os.path.exists(SAVES_DIR):
                    os.mkdir(SAVES_DIR)
                self.game_files = [
                    file for file in os.listdir("saves") if file.endswith(".pi")
                ]
    ...
    def execute_command(self, command: str):
        ...
        match cmds:
            ...
            case ["info"]:
                ...
                print(
                    f"Number of active trades: {self.g.num_active_trade_indices()}/{TRADES_BOUND + len(self.g.achievements)}"
                )
                print(
                    "Sum of all numbers (incl. duplicates):",
                    self.g.sum_of_all_numbers(),
                )
            case ["test"]:
                pass
            case ["save"]:
            ...
            case ["ach" | "achievements"]:
                if not self.g.achievements:
                    ...
                    return
                print(
                    f"*** Completed achievements ({len(self.g.achievements)}/{len(ACHIEVEMENTS)}) ***"
                )
